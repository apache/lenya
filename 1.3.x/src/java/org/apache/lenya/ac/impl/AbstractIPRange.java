...
public abstract class AbstractIPRange extends AbstractGroupable implements IPRange {
   /*
    * FIXME by zisch@dals.ch: Fixed this class for IPv6. However there are still some general flaws, partly coming from the IPRange interface. A redesign of (Abstract/File)IPRange and it's helper class org.apache.lenya.net.InetAddressUtil would be a good idea. Some problems of this implementation are: - The whole initialization seems flawed. Objects can be in an unitialized state and the class seems not to be aware of this. - Network-address and -mask can be set independently. Therefore it cannot be enforced that these have the same size (i.e. that both are IPv4 or both are IPv6). This shows up in InetAddressUtil.contains(...), where in a case of mismatch there is no good way to inform the user about the problem. This should be done once when the AbstractIPRange object is initialized. - Unless this functionality would be needed by other parts of Lenya or external software (which seems not to be the case ;-), InetAddressUtil should be removed (resp. deprecated) altogether, because it's
   ...
   public AbstractIPRange() {
      try{
         ...
         /*
          * FIXME? by zisch@dals.ch: Is it safe to ignore the exception and just leave the IPRange uninitialized!?
          */
      }
   }
   /**
   ...
   public AbstractIPRange(String id) {
      /*
       * FIXME? by zisch@dals.ch: Is it safe not to call the default constructor and just leave the IPRange uninitialized!?
       */
      setId(id);
   }
   ...
   public static boolean isValidSubnet(InetAddress networkAddress, InetAddress subnetMask) {
      /*
       * FIXME? by zisch@dals.ch: Is this method really necessary (what for?) and (if so) shouldn't it be an internal (private) utility-method??
       */
   ...
   public boolean contains(Machine machine) {
      /*
       * FIXME? by zisch@dals.ch: Maybe some mapping between IPv4/v6 should be done here, p.e. for the localhost (see the javdoc comment above)? (I'm not a TCP/IP-guru, so I'm not sure about this. ;-)
       */
