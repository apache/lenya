		...
		<xsl:choose>
	        <xsl:when test="not($outputType = 'WML') and not($outputType = 'PALM')">
	        	<xsl:choose>
					...
					<xsl:otherwise>
<!--
	OpenOffice 2.0 now wraps internals in text element
-->
...
<!-- WML         <xsl:value-of select="normalize-space(.)"/> -->
	...
	<!-- add in annotations - art -->
	<xsl:if test="count(office:annotation) &gt; 0">
		...
		<xsl:attribute name="author">
			<xsl:value-of select="@office:author"/>
		</xsl:attribute>
		<xsl:attribute name="create-date">
			<xsl:value-of select="@office:create-date"/>
		</xsl:attribute>
    ...
    <xsl:template name="create-common-link">
        ...
        <xsl:choose>
			...
			<xsl:if test="contains(@xlink:href,'info://')">
				<xsl:text>Info Link Here</xsl:text>
			</xsl:if>
			...
			<xsl:value-of select="@xlink:title"/>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="contains(@xlink:href, 'Pictures/')">
                        ...
                        <xsl:attribute name="src"><xsl:value-of select="concat($jaredRootURL,concat($document_name,'.odt/Pictures/', substring-after(@xlink:href, 'Pictures/'), $optionalURLSuffix))"/></xsl:attribute>
                    </xsl:when>
<!--                    Due to a XT bug no DOS ':' before DRIVE letter is allowed, it would result in a unkown protoco exception, but a file URL for a DOS
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:attribute name="src"><xsl:value-of select="concat($metaFileURL, @xlink:href, $optionalURLSuffix)"/></xsl:attribute>
                    </xsl:otherwise>
-->
					<xsl:otherwise>
                        <xsl:attribute name="src"><xsl:value-of select="@xlink:href"/></xsl:attribute>
