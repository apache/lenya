...
/**
 * LDAP user.
 * @version $Id: LDAPUser.java 473842 2006-11-12 01:15:20Z gregor $
 */
public class LDAPUser extends FileUser {
    ...
    public boolean existsUser(String ldapId) throws AccessControlException {
        ...
        try {
            ...
            log.info("LDAPUser.existsUser() got exception while looking up id [" + ldapId
                    + "], so will return false", e);
            exists = false;
        } catch (Exception e) {
            if (log.isDebugEnabled())
     ...
     * 
     * The current ldapId is queried in the directory, in order to retrieve
     * additional information, such as the user name. In current implementation,
     * only the user name is actually retrieved, but other attributes may be
     * used in the future (such as groups ?)
     * 
     * Note: if the user entry could not be retrieved, initialize the attributes
     * to empty string (they are optional anyway), but do not throw an
     * exception.
     */
    protected void initialize() {
        try {
            ...
            if (entry != null) {
                ...
                if (log.isDebugEnabled())
                    log.debug("initialize() got entry, going to look for attribute " + usrNameAttr
                ...
                if (attributes != null) {
                    ...
                    if (userNames != null) {
                        for (NamingEnumeration nenum = userNames.getAll(); nenum.hasMore(); nenum
                                .next()) {
                            name.append((String) userNames.get());
                        }
                    }
                    for (NamingEnumeration ids = attributes.getIDs(); ids.hasMore(); ) {
                        String id = (String) ids.next();
                        Attribute attr = attributes.get(id);
                        Set values = new HashSet();
                        for (NamingEnumeration valueEnum = attr.getAll(); valueEnum.hasMore(); ) {
                            String value = String.valueOf(valueEnum.next());
                            values.add(value);
                        }
                        try {
                            setAttributeValues(id, (String[]) values.toArray(new String[values.size()]));
                        } catch (AccessControlException e) {
                            log.error("Could not set LDAP user attribute values: ", e);
                        }
                    }
                }
            }
            ...
            if (log.isDebugEnabled())
                log.debug("initialize() set name to " + name);

        } catch (IOException e) {
            log.warn("LDAPUser.initialize() can not read the user name for the id [" + ldapId
                    + "], this is probably a setup error of your user entry.", e);
        } catch (NamingException e) {
            log.warn("LDAPUser.initialize() can not read the user name for the id [" + ldapId
                    + "], this is probably a setup error of your user entry.", e);
        }
     ...
     * configured properties, or by lookup of this ID in the directory. This
     * principal then attempts to authenticate against the directory with the
     * provided password.
     * 
    ...
    private SearchResult getDirectoryEntry(String userId) throws NamingException, IOException {
        ...
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("searching object " + objectName + " filtering with " + searchFilter
            ...
            if (results != null && results.hasMore()) {
                ...
                // try {
                // if (results.hasMore()) {
                // log.warn("Found more than one entry in the directory for user
                // " + userId + ". You probably should deactivate recursive
                // searches. The first entry was used as a work-around.");
                // }
                // }
                // catch (javax.naming.PartialResultException e) {
                // if (log.isDebugEnabled())
                // log.debug("Catching and ignoring PartialResultException, as
                // this means LDAP server does not support our sanity check");
                // }

            }
        } catch (NamingException e) {
            if (log.isDebugEnabled())
                log.debug("NamingException caught when searching on objectName = " + objectName
        ...
        if (domainProp != null && domainProp.trim().length() > 0) {
            ...
            if (isSubtreeSearch()) {
                ...
                if (entry.isRelative()) {
                    ...
                    // if the item is found followin a referral an URL string is
    ...
    private String constructPrincipal(String userId) {
        ...
        if (baseDn != null && baseDn.length() > 0) {
            ...
            log
                    .warn("constructPrincipal() read a deprecated format in ldap properties, please update");
