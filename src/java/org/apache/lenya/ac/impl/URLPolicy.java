...
/* $Id: URLPolicy.java 473841 2006-11-12 00:46:38Z gregor $  */
...
public class URLPolicy implements Policy {
    ...
    public static Role getVisitorRole(AccreditableManager manager) throws AccessControlException {
        return getExistingRole(manager, VISITOR_ROLES);
    }
    ...
    public static Role getAdministratorRole(AccreditableManager manager)
        ...
        return getExistingRole(manager, ADMINISTRATOR_ROLES);
    }
    ...
    public static Role getAuthorRole(AccreditableManager manager) throws AccessControlException {
        return getExistingRole(manager, AUTHOR_ROLES);
    }

    protected static Role getExistingRole(AccreditableManager manager, String[] roleIdArray) throws AccessControlException {
        Role authorRole = null;
        Role[] roles = manager.getRoleManager().getRoles();
        List roleIds = Arrays.asList(roleIdArray);
        for (int i = 0; i < roles.length; i++) {
            if (roleIds.contains(roles[i].getId())) {
                authorRole = roles[i];
            }
        }
        return authorRole;
    ...
    public Accreditable[] getAccreditables(Role role) {
        ...
        for (int i = 0; i < this.policies.length; i++) {
            Accreditable[] accrs = this.policies[i].getAccreditables(role);
