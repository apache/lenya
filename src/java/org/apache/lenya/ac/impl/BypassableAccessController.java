...
/* $Id: BypassableAccessController.java 473841 2006-11-12 00:46:38Z gregor $  */
...
public class BypassableAccessController extends DefaultAccessController {
    ...
    public boolean authorize(Request request, ErrorHandler handler) throws AccessControlException {
        ...
        while (!authorized && i < publicMatchers.size()) {
            getLogger().debug(
                    "Trying pattern: [" + publicMatchers.get(i) + "] with URL [" + uri + "]");
        ...
        if (!authorized) {
            authorized = super.authorize(request, handler);
        }
