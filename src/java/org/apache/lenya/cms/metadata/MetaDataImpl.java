 ...
 * meta-data requirements, a set of meta-data may contain up to two namespaces,
 * one for "elements" and one for "terms". A metadata implementation requiring
 * only one namespace can ignore the "terms".
 * 
 * @version $Id$
 */
public abstract class MetaDataImpl extends AbstractLogEnabled implements
    MetaData {
  ...
  /**
   * Creates a new instance of metadata, reading any existing values from the
   * source URI.
   * 
   * @param sourceUri
   *          The source URI.
  ...
  /**
   * Elements to be used in this meta-data. A meta-data implementation which
   * does not have a fixed list of elements may return an empty array, and
   * override useFixedElements()
   * 
  ...
  /**
   * Terms to be used in this meta-data. A meta-data implementation requiring
   * only one namespace may ignore terms and return an empty array.
   * 
  ...
  /**
   * Determine if the meta-data should consist of a fixed list of known
   * attributes, or whether arbitrary names can be used.
   * 
        ...
        for (int type = 0; type < namespaces.length; type++) {
          ...
          if (useFixedElements()) {
            ...
            for (int i = 0; i < elementNames.length; i++) {
              Element[] children = helper.getChildren(metaElement,
                  elementNames[i]);
            ...
            for (int i = 0; i < elements.length; i++) {
              loadElementValues(maps[type], elements[i].getTagName(), helper
                  .getChildren(metaElement, elements[i].getLocalName()));
            }
          }
        }
      }
  ...
  /**
   * Retrieve an element (and its values) from the DOM and add the element to
   * the map
   */
  ...
  public void save() throws DocumentException {
    ...
    if (getLogger().isDebugEnabled())
      getLogger().debug(
          "MetaDataImpl::save() called, sourceUri [" + sourceUri + "]");
    ...
    try {
      ...
      Node[] children = (Node[]) childNodes
          .toArray(new Node[childNodes.size()]);
   ...
   */
  private void writeElementValues(NamespaceHelper helper, Element father,
      Map elementMap, String elementName) {
   ...
   *           if the meta-data's source can not be found
   ...
   */
  protected Document getDocument() throws ServiceException,
      SourceNotFoundException, ParserConfigurationException, SAXException,
      IOException {
  ...
  protected Element getMetaElement(Document document) throws DocumentException {
    ...
    try {
      NamespaceHelper namespaceHelper = new NamespaceHelper(
          PageEnvelope.NAMESPACE, PageEnvelope.DEFAULT_PREFIX, document);
      Element documentElement = namespaceHelper.getDocument()
          .getDocumentElement();
   ...
   *           if an error occurs.
   */
  protected Element getLocalMetaElement(Document document)
      throws DocumentException {
    ...
    try {
      NamespaceHelper namespaceHelper = new NamespaceHelper(
          PageEnvelope.NAMESPACE, PageEnvelope.DEFAULT_PREFIX, document);
      metaElement = namespaceHelper.getFirstChild(topMetaElement,
          getLocalElementName());
  ...
  /**
   * @see org.apache.lenya.cms.metadata.MetaData#addValue(java.lang.String,
   *      java.lang.String)
   */
